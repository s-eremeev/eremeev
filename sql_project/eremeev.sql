...
select	apt.city ->> lang() as "город", apt.cnt as "количество аэропортов",
			string_agg (apt.airport_name ->> lang(), ', ' order by apt.airport_name ->> lang()) as "аэропроты"
from 
...
with ratio as
	(select distinct	f.aircraft_code,
							round ((count (f.flight_id) over (partition by f.aircraft_code)) *100.0 / (select count (*) from flights), 2)
	from flights f)
...
from
	(with amnt as 
		...
		coalesce (min (tf.amount) filter (where tf.fare_conditions = 'Business') over (partition by tf.flight_id, city, tf.fare_conditions), '0') as max_amount 	
	...
	coalesce (min (amnt.max_amount) filter (where amnt.fare_conditions = 'Business') over (partition by amnt.flight_id, amnt.city), '0') as max_bus,
	coalesce (max (amnt.max_amount) filter (where amnt.fare_conditions = 'Economy') over (partition by amnt.flight_id, amnt.city), '0') as max_eco
	from amnt) as res
