...
-- Общий принцип: t_XXX - алиасы таблиц, c_XXX - алиасы колонок

-- ГЛАВНАЯ ТАБЛИЦА с выборкой по всем критериям
WITH T_MAIN AS
 -- Отбор данных за 14 дней
(
	SELECT
						...
						(
							...
							FROM
								(
									...
									WHERE
										REPORT_DATE <= TO_DATE('2023-11-01', 'yyyy-mm-dd') - 1
										AND REPORT_DATE >= TO_DATE('2023-11-01', 'yyyy-mm-dd') - 7
								) AS T_RANGE_DATE
						) AS T_MAX_DATE
					WHERE
...
WITH SELECT_FLAG(FLAG_) AS (VALUES(0)), -- выбор расчётной даты для каждого региона: 0 - последняя дата, 1 - предпоследняя дата
BORDER_DAYS(I, DAYS) AS (VALUES (0, 7), (1, 4), (2, 10)), -- "отступы": max дней от даты дайджеста до даты последнего отчёта (7), границы интервала для даты предпоследнего отчёта (от 4 до 10)
MIN_SLOT_LENGTH(MM) AS (VALUES(5)), T_MAIN AS -- продолжительность "короткого" слота, мин
 -- Отбор данных за 14 дней
(
	SELECT
		C_ID,
...
-- рабочий вариант без моих полей, только Морев
WITH SELECT_FLAG(FLAG_) AS (VALUES(0)), -- выбор расчётной даты для каждого региона: 0 - последняя дата, 1 - предпоследняя дата
BORDER_DAYS(I, DAYS) AS (VALUES (0, 7), (1, 4), (2, 10)), -- "отступы": max дней от даты дайджеста до даты последнего отчёта (7), границы интервала для даты предпоследнего отчёта (от 4 до 10)
T_MAIN AS
 -- Отбор данных за 14 дней
