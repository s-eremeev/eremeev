with
	select_flag(flag_) as (values(1)), 									-- выбор расчётной даты для каждого региона: 0 - последняя дата, 1 - предпоследняя дата
	border_days(i, days) as (values (0, 7), (1, 4), (2, 10), (3, 14)), 	-- "отступы": max дней от даты дайджеста до даты последнего отчёта (7), границы интервала для даты предпоследнего отчёта (от 4 до 10)
	min_slot_length(mm) as (values(5)),									-- продолжительность "короткого" слота, мин
	t_MAIN as
	-- Отбор данных за 14 дней
	(select distinct c_ID, c_REG, c_DATE, mo_oid, mo_short_name, sp_oid, sp_name, mp_dolgnost, mp_fio, c_DATE_REP, slots_type, c_SLOTS_C, c_SLOTS_B, c_STAVKA, slot_length,  visits_absence
	 	from
		-- Отбор из основной таблицы
	 	(select c_ID, c_REG, c_DATE, mo_oid, mo_short_name, sp_oid, sp_name, mp_dolgnost, mp_fio, slots_type,
		 		cast(slots_created as decimal(10, 0)) as c_SLOTS_C,
		 		cast(slots_booked as decimal(10, 0)) as c_SLOTS_B,
		 		case
				...
				case
		 		...
		 		case
		 			when replace (slot_length, ',', '.') ~ '^[0-9]+.[0-9]+$' then cast(replace (slot_length, ',', '.') as decimal(10, 0))
		 			else 0
		 		end as slot_length,
		 		case
		 			when visits_absence ~ '^[0-9]+$' then cast(visits_absence as int)
		 			else 0
		...
		and (slot_length ~ '^[0-9]+.[0-9]+$' or slot_length ~ '^[0-9]+,[0-9]+$')
		and visits_absence ~ '^[0-9]+$'
		) as t_MAIN_tmp, border_days
	where c_DATE_REP < c_DATE and c_DATE_REP >= c_DATE - (select days from border_days where i = 3))
...
select c_ID, c_DATE, c_REG_0, mo_oid_0, mo_short_name_0, sp_oid_0, sp_name_0, mp_dolgnost_0,
	...
	left join
		...
		sum(c_SLOTS_C) as SHORT_SLOTS_COUNT from t_MAIN, min_slot_length
