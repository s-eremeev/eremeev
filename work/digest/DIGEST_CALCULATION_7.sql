...
FROM(
	...
	REPLACE(LOWER(DP.CORRECT_PROFESSION_NAME), '  ', ' ') AS MP_DOLGNOST,
	RR.MP_ID,
	...
	CASE
		...
		WHEN LEFT(RR.DATE_REPORT, 10) ~ '^([0-9]{1}|[0-9]{2})/([0-9]{1}|[0-9]{2})/[2][0][2][3-9]$' THEN TO_DATE(RR.DATE_REPORT, 'dd.mm.yyyy')
	...
	ON RR.MP_DOLGNOST = DP.ORIGIN_PROFESSION_NAME
	WHERE
...
12031
	)) AS T0 WHERE T0.C_DATE_REP < T0.C_DATE AND T0.C_DATE_REP >= T0.C_DATE - 14 AND T0.SLOT_LENGTH > 0 AND T0.SLOT_LENGTH > 0
)
